...
if ! ipset list -n|command grep -q "$IPSET_BLACKLIST_NAME"; then
	...
	echo >&2 "# ipset create $IPSET_BLACKLIST_NAME -exist hash:net family inet hashsize ${HASHSIZE:-131072} maxelem ${MAXELEM:-262144}"
	exit 1
    fi
    if ! ipset create "$IPSET_BLACKLIST_NAME" -exist hash:net family inet hashsize "${HASHSIZE:-131072}" maxelem "${MAXELEM:-262144}"; then
...
create $IPSET_TMP_BLACKLIST_NAME -exist hash:net family inet hashsize ${HASHSIZE:-131072} maxelem ${MAXELEM:-262144}
create $IPSET_BLACKLIST_NAME -exist hash:net family inet hashsize ${HASHSIZE:-131072} maxelem ${MAXELEM:-262144}
EOF
